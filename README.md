...
The default mapping is `zy` for copying and `zp` for pasting, and can be followed by any motion or text
object. For instance:

- `zyiw` => copy word into system clipboard
- `zyi'` => copy inside single quotes to system clipboard
- `zpi'` => paste inside single quotes from system clipboard

In addition, `zyy` is mapped to copy the current line directly.

The sequence `zpp` is mapped to paste the content of system clipboard to the
next line.
